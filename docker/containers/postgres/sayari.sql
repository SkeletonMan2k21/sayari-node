...
(
  ...
  total_reach integer UNSIGNED NOT NULL DEFAULT 0,         -- unsigned, can have zero reach but not negative
  num_answers integer UNSIGNED NOT NULL DEFAULT 0,         -- unsigned, can't have less than zero answers posted
  num_questions integer UNSIGNED NOT NULL DEFAULT 0,       -- unsigned, can't have less than zero questions posted
...
(
  id SERIAL PRIMARY KEY,
  question_id integer UNSIGNED NOT NULL REFERENCES questions(id),
  user_id integer UNSIGNED NOT NULL REFERENCES users(id),
  body text NOT NULL,
  posted_date timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
(
  ...
  parent_type_id integer UNSIGNED NOT NULL REFERENCES parent_types (id),
  body text NOT NULL,
  posted_date timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, 
  deletion_date timestamp NULL
);
...
(
	question_id integer UNSIGNED NOT NULL REFERENCES questions (id),
	tag_id integer UNSIGNED NOT NULL REFERENCES tags (id),
	added_date timestamp NOT NULL // Index
);

-- Unique composite index guarantees each tag is attached to a single question only once (will also work as an index for querying tags by question_id)
-- Single column index on added_date allows for sorting/reporting tags by date (e.g. list of tags added to question X within Y days)
CREATE UNIQUE INDEX uq_questions_tags_question_id_tag_id ON questions_tags (question_id, tag_id);
